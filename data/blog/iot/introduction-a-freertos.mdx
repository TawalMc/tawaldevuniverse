...
_Cet article n'est pas pour un débutant en système embarqué mais être juste à l'aise avec les micronctrôleurs est recommandée._

![FreeRTOS-banner](https://miro.medium.com/max/2880/1*elMEPVMRFNKdpvVvZjLwoQ.png)

# Parlons de FreeRTOS

[FreeRTOS](https://www.freertos.org/) ou Free Real Time Operating est un kernel(noyau) multitâche (multitask) pour microcotrôleur.
...
Actullement je suis encore en train de lire le [guide tutoriel](https://www.freertos.org/fr-content-src/uploads/2018/07/161204_Mastering_the_FreeRTOS_Real_Time_Kernel-A_Hands-On_Tutorial_Guide.pdf) sur FreeRTOS qu'on peut
...
- Un système de timer, pour l'exécuter d'une fonction à une date donnéé dans le futur
...
- Pleins de démos sur son utilisation et le code source est totalement ouvert et gratuit à toute utilisation. Pour des options et fonctionnalités vraiment avancés des versions commerciales sont disponibles - ( _😂un ami à moi cherchera partout en gratuité la version commerciale, c'est plus fort que lui😂_ )
...
que les différentes allocation de mémoires se feront. Différents algorithmes sont alors utilisés en fonction de vos besoins: `heap 1` à `heap 5`.
...
N'hésitez pas à me faire des retours dans mon mail ou sur mon Twitter ou Linkedin dont les références sont juste en dessous.
