...
draft: false
summary: "Dans cet article nous irons un peu plus en profondeur ce qui concerne l'utilisation des tâches de FreeRTOS en Arduino"
images:
  [
...
Reamarquez ici que nos tâches on un prototype bien connu: elles prennent en paramètre un `pointeur générique` _(peut pointer ensuite sur n'importe quel type de données)_. Certes
...
Quand nous passons uen carte, son ID s'affiche plusieurs fois dans le serial monitor
...
elle l'exécute à chaque fois. Ramenez juste la priorité de la tâche `blinking` à 2 et vous verrez qu'on aura le même fonctionnement que le premier test
(la led s'allume et s'éteint).

## Test2: RFID-RC522 et Serial Monitor + LED (2 tâches + différentes priorités)
...
gestionnaire de tâche en ce moment comprendra/verra _(qu'est ce qu'il y peut, même les Avengers avaient échoué le première fois)_ la tâche `blinking` en attente.
...
dans le blocked state pour permettre aux tâches de priorités inférieures de s'exécuter.

## Test 3: RFID-RC522 + Serial Monitor + LED (3 tâches)

Bref ici c'est un exercice pour la prochaine fois:

- Nous devons créer trois tâches: l'une pour receuillir l'ID de la carte, l'autre pour afficher cet ID, l'autre pour le blinking.
...
`de passer l'ID à la tâche qui l'affiche uniquement quand il y a ID`. En lisant l'article introductif sur FreeRTOS vous verrez le mot.
...
- Nous pouvons explicitement indiquer l'importance de l'exécution d'une fonction, quand elle peut s'exécuter explicitement sans se soucier de `millis()` ou du retour d'une fonction
  ...
  ou simplement cliquer sur `Discutons en sur Twitter •`.
