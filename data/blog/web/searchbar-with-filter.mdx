...
const SearchbarSelectCheckbox = (props: SearchbarSelectCheckboxType) => {
   const {data, groupBy} = props;
    ...
    let entitiesGroupAsMap = useMemo(() => {
    let setEntity = new Set(data.map(groupBy));
    let mapEntity = new Map<string, string>();

    setEntity.forEach((v) => {
    if (v !== undefined && v !== null) mapEntity.set(v, v);
...
- nous parcourons le tableau de données avec un `map`, et grâce à=au callback (`̀groupBy`)
