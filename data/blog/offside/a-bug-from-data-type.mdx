...
Okeuh. And after? Simple we use data types to declare our variables. Something like this:
...
And each data type can hold (a maximum) `amount of data` (🤪). Each of them has a number of `bytes` that
can be used to store them depending on the target architecture. And when we are in the embedded we must
take care of them. We don't work on computer with GigaBytes as storage, and so the greater are our data
the more bytes we consume and so more spaces. In addition, they are some side effects when we don't use right
data types and one of them is: `data type overflow`.

## Overflow? Like a water over-flow a container

You know when water overflows a container, it simply `sinks`, but the container is just filled to it. No more.
It's let's say the same thing which we'll happen if your data is bigger than your data types: it `cut-off`.
So you try to store in `int` a value of `123456` on `Arduino`, not good.

Recently I worked on project and face this issue and only my computer knows how much time I take to figure it out.

## My Problem: store/get millis value

You know with Arduino, `millis()` return the number of `milliseconds` since our `program is running`...
And the best type to store the `value returned` by `millis()` is `unsigned long`. Why?
Because, `millis()` can return data like `120'000` which equals to `2 (minutes)`. So imagine you try to store
...
This piece of code execute correctly one time only. Why ? Simple:

1- After 02 minutes the expression `(millis() - duration_between_req >= MAX_TIME)` is true so
my code execute first time the action and then put in `duration_between_req` the value returned by `millis()`
...
each second (less than 01 second) the program execute the if statement body...

# Yes, it is so fun

I spent at least 30 minutes to understand my code (**my code hum, a code that I write myself**) and let's say, Arduino IDE don't help anymore...
...
**what will happen? What do you hope? What magic will occur? Ask your code...**

I share some tips that I used in my daily tasks and I hope you've some others tips that you want to share
with us. I'm open to advises, and my social accounts (below) are there for this purposes...
